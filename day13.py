...
def part2():
    data = open("day13_input.txt","r").readlines()
    layer = dict()
    severity = 0

    for line in data:
        line = line.strip().split(": ")
        layer[int(line[0])] = int(line[1])

    caught = False
    for delay in xrange(10, 10**7):
        caught = False
        for i in layer.keys():
            if (i+delay) % (2* layer[i] - 2) == 0:
                caught = True
                break
        if not caught:
            print "Part 2:\t\t" ,delay
...
def part1():
    ...
    for x in range(len(layer)):
        if (x+3878062) % ((layer[x] -1)*2) == 0:
            #print "Caught in Layer: ", x, "Severity: ", x,depth, " = " ,x*depth
            severity += layer[x] * x
