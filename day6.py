...
def Main():
    ...
    cycles= 0
    numberBlocks = len(state)

    while tuple(state) not in seenStates:
        seenStates[tuple(state)] = cycles
        maxSize = max(state)
        maxIndex = state.index(maxSize)
        #print(maxSize, maxIndex)
        state[maxIndex] = 0

        for x in range(0,maxSize):
            state[(maxIndex + x +1) % numberBlocks] += 1
